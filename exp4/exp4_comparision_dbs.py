...
import os
import torch
from sklearn.neighbors import NearestNeighbors  # ★ 新增：用於 DBS
...
class SingleRunExperiment:
    ...
    # ----------------------------------------------------------------------
    # 原本的 cluster_sampling 已移除，改為密度基礎抽樣 (DBS)
    # ----------------------------------------------------------------------
    def dbs_sampling(self, data, num_samples):
        """
        使用密度基礎抽樣 (Density-Based Sampling)：
        1. 先對所有樣本做 embedding
        2. 使用 NearestNeighbors 找出每個樣本與其最近幾個鄰居的距離
        3. 將距離總和當作 density score，越小表示越密集
        4. 取前 num_samples 筆作為抽樣結果
        """
        texts = [x["text"] for x in data]
        logging.info(f"Generating embeddings for density-based sampling: {len(texts)} samples...")
        emb = self.st_model.encode(texts, batch_size=32, show_progress_bar=True)

        # 計算最近鄰距離
        nn = NearestNeighbors(n_neighbors=min(5, len(emb) - 1)).fit(emb)
        distances, _ = nn.kneighbors(emb)

        # 以各樣本的距離總和當作密度分數
        density_scores = np.sum(distances, axis=1)

        # 依照分數排序，取前 num_samples 筆
        sorted_indices = np.argsort(density_scores)
        subset_indices = sorted_indices[:num_samples]

        subset = data.select(subset_indices)
        return subset
    ...
    def compute_quad_partial_match(self, gold_data, pred_data):
        ...
        quad_score = (ac_score + lb_score + span_lcs + ot_lcs)/4
        """
        import numpy as np
        n = len(gold_data)
        totP, totR = 0, 0
    ...
    def run(self):
        start_t = time.time()
        self.load_data()
        logging.info(f"Sampling {TRAIN_SIZE} from train set with DBS...")

        # ★ 這裡改用 dbs_sampling
        self.train_dataset = self.dbs_sampling(self.full_train_dataset, TRAIN_SIZE)
